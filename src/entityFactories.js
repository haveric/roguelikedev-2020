...
EntityFactories.targetDummy = (x, y) => {
    const entity = new Actor(x, y, "Target Dummy", "It just stands there, mocking you.", new Sprite("targetDummy", "654321"));
...
EntityFactories.spacePirate = (x, y) => {
    const entity = new Actor(x, y, "Space Pirate", "A Pirate. In space! He has a menacing look.", new Sprite("spacePirate", "cc0000"));
...
EntityFactories.attackDog = (x, y) => {
    const entity = new Actor(x, y, "Attack Dog", "Faithful companion to pirates; looking mighty hungry for flesh.", new Sprite("attackDog", "654321"), false);
...
EntityFactories.automatedTurret = (x, y) => {
    const entity = new Actor(x, y, "Automated Turret", "Deadly Turret, pointing straight at you and defending whatever is nearby from you.", new Sprite("automatedTurret", "222222"));
...
EntityFactories.medkit = (x, y) => {
    const entity = new Item(x, y, "Medkit", "Can be used to heal a small amount of health.", new Sprite("medkit", "7f00ff"));
    entity.setComponent("consumable", new HealingConsumable(entity, 4, "use"));
    return entity;
};

EntityFactories.laserCharge = (x, y) => {
    const entity = new Item(x, y, "Laser Charge", "Shoots a laser at the nearest visible enemy.", new Sprite("laserCharge", "ffff00"));
    entity.setComponent("consumable", new LaserDamageConsumable(entity, 20, 5));
    return entity;
};

EntityFactories.confuseRay = (x, y) => {
    const entity = new Item(x, y, "Confuse Ray", "Confuses the weak-minded temporarily.", new Sprite("confuseRay", "cf3fff"));
    entity.setComponent("consumable", new ConfusionConsumable(entity, 10));
    return entity;
};

EntityFactories.grenade = (x, y) => {
    const entity = new Item(x, y, "Grenade", "Standard military issue explosive device. Pull pin and throw.", new Sprite("grenade", "ff0000"));
    entity.setComponent("consumable", new GrenadeDamageConsumable(entity, 12, 3));
    return entity;
};

EntityFactories.resurrectionInjector = (x, y) => {
    const entity = new Item(x, y, "Resurrection Injector", "Brings dead things back to life. What looks like possible side effects has been scratched out.", new Sprite("resurrectionInjector", "00ee00"));
